...
public class AutoDriveProfileGyro extends CommandBase {
	...
	private PID pid = new PID(Profiler_Constants_DriveTrain.Kp, Profiler_Constants_DriveTrain.Ki,
			Profiler_Constants_DriveTrain.Kd);

	public AutoDriveProfileGyro(double dist_inches, double vel_inches_sec, double heading_deg,
			double accel_inches_sec_sec) {
	...
	public void initialize() {
		...
		mp = new MotionProfiler(_absDistance, Profiler_Constants_DriveTrain.profileInitVelocity, _cruiseSpeed,
				Profiler_Constants_DriveTrain.profileDriveAccelration);
	...
	public void execute() {
		...
		String msg = String.format(
				...
				subDriveTrain.getEncoderPosRight_Inches(), currentHeading, turnValue, pidVal, pid.getDError(),
				finalThrottle, pid.getIError(), pid.getPError(), currentHeading);
		...
		// Robot.subDriveTrain.getLeftEncoderDist());
		// SmartDashboard.putNumber("Drive Right Encoder: ",
		// Robot.subDriveTrain.getRightEncoderDist());

		subDriveTrain.Drive((finalThrottle * _distanceSignum), turnValue);

		// if (deltaTime > _abortTime && Robot.subDriveTrain.getEncoderAvgDistInch() ==
		// 0) {
	...
	protected void end() {
		...
		System.out.println("AutoDriveProfileGyro is Finished Left Encoder: " + subDriveTrain.getEncoderPosRight_Inches()
				+ " Right Encoder: " + subDriveTrain.getEncoderPosRight_Inches());
		System.err.println(String.format(
